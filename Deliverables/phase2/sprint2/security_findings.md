...
The ZAP baseline scan performs **passive analysis only** against the live stack.
Active-scan findings (SQL injection probes, XSS payloads) do not appear in a
baseline run. The DAST report is published as the `zap-report` pipeline artifact
on every push to `develop`/`main`.

| Finding class | Expected result | Notes |
|---------------|-----------------|-------|
| SQL Injection | Not triggered | Spring Data JPA parameterised queries; no raw SQL |
| XSS | Not triggered | REST API returns `application/json`; no HTML rendering |
| CSRF | IGNORE | Stateless JWT — not applicable |
| Missing security headers | WARN | HSTS set by nginx; `X-Content-Type-Options` served by Spring |
| Information disclosure | WARN | Stack traces suppressed by `GlobalExceptionHandler` |
...
| Transport | JaCoCo TCP server (`output=tcpserver,port=6300,address=*`) — no file I/O inside the container needed |
...
ZAP Baseline Scan ──► exercises REST endpoints (auth, files, purchases)
        │
        ▼
mvn jacoco:dump  ──► pulls execution data from TCP:6300 → jacoco-iast.exec
        │
docker compose down
        │
        ▼
